...
import com.abdalla.bushnaq.mercator.renderer.AtlasManager;
...
import com.abdalla.bushnaq.mercator.universe.sim.SimNeed;
...
public class Info {
	...
	private void update(final Universe universe, final GLProfiler profiler) {
		if (profiler != null && profiler.isEnabled() && debugTimer.getTime() > 1000) {
			...
			if (sceneClusterManager != null) {
				{
					final int count = sceneClusterManager.visibleStaticGameObjectCount;
					final int totalCount = sceneClusterManager.staticModelInstances.size;
				...
				{
					stringBuilder.setLength(0);
					final int count = sceneClusterManager.visibleDynamicGameObjectCount;
					final int totalCount = sceneClusterManager.dynamicModelInstances.size;
					stringBuilder.append(count).append(" / ").append(totalCount);
				...
				{
					stringBuilder.setLength(0);
					final int count = sceneClusterManager.visibleStaticLightCount;
					final int totalCount = sceneClusterManager.environment.get(PointLightsAttribute.class, PointLightsAttribute.Type).lights.size;
					stringBuilder.append(count).append(" / ").append(totalCount);
					updateNameAndValue("Static PointLights", stringBuilder.toString(), VARIABLE_LABEL);
				}
				{
					stringBuilder.setLength(0);
					final int count = sceneClusterManager.visibleDynamicLightCount;
					final int totalCount = sceneClusterManager.environment.get(PointLightsAttribute.class, PointLightsAttribute.Type).lights.size;
					stringBuilder.append(count).append(" / ").append(totalCount);
	...
	public void update(final Universe universe, final Object selected, final SceneManager sceneClusterManager) throws Exception {
		this.sceneClusterManager = sceneClusterManager;
