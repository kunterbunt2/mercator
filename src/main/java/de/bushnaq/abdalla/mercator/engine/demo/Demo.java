...
public class Demo {
    ...
    private final List<TextFormat>        text            = new ArrayList<>();
...
//    }
    ...
    private void export(final String fileName, final List<TextFormat> Strings) throws IOException {
        ...
        for (final TextFormat demoString : Strings) {
...
//            executeTasks(deltaTime);
            ...
            if (text.isEmpty()) {
                text.add(new TextFormat("Mercator", gameEngine.getAtlasManager().bold128Font, Color.WHITE));
                text.add(new TextFormat("A computer game implementation of a closed economical simulation.", gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
                text.add(new TextFormat(String.format("The current world is generated proceduraly and includes %d cities, %d factories, %d traders and %d sims.", gameEngine.universe.planetList.size(), gameEngine.universe.planetList.size() * 2, gameEngine.universe.traderList.size(), gameEngine.universe.simList.size()), gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
                text.add(new TextFormat(String.format("There exist %d static models, %d dynamic models, %d audio producers.", gameEngine.renderEngine.staticGameObjects.size, gameEngine.renderEngine.dynamicGameObjects.size, gameEngine.audioEngine.getNumberOfAudioProducers()), gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
                text.add(new TextFormat("The amount of wealth in the system, including products and money is constant at all times. ", gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
                text.add(new TextFormat("Factories pay wages to sims to produce goods that are sold on a free market.", gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
                text.add(new TextFormat("Some sims are traders that buy products in one city and sell them with profit in another city.", gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
                text.add(new TextFormat("All sims have needs that they need to fulfill else they die.", gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
                text.add(new TextFormat("All sims have cravings that they need to fulfill to keep their satisfaction level up.", gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
                text.add(new TextFormat("All sounds are generated by a openal based audio render engine for libgdx supporting procedurally generated audio using HRTF.", gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
                //				demoText.add(new DemoString("Demo song is 'abyss' by Abdalla Bushnaq.", render2DMaster.atlasManager.demoMidFont));
                text.add(new TextFormat("Work in progress...", gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
                text.add(new TextFormat("Developed using libgdx and gdx-gltf open source frameworks.", gameEngine.getAtlasManager().demoMidFont, Color.WHITE));
        ...
        for (Integer neighbors : planetNeighbors.keySet()) {
            if (neighbors > 3) {
                ...
                for (Planet planet : planetList) {
                    ...
                    if (firstPlanet) {
                        firstPlanet = false;
                        gameEngine.getRenderEngine().getScheduledEffectEngine().add((ScheduledTask) new MercatorPositionCamera<>(gameEngine, zoomIndex, planet.name));
                        gameEngine.getRenderEngine().getScheduledEffectEngine().add((ScheduledTask) new RotateCamera<>(gameEngine, angle));
                        gameEngine.getRenderEngine().getScheduledEffectEngine().add((ScheduledTask) new PauseTask<>(gameEngine, firstSecondsDelta));
                    } else {
                        gameEngine.getRenderEngine().getScheduledEffectEngine().add((ScheduledTask) new MercatorPositionCamera<>(gameEngine, zoomIndex, planet.name));
                        gameEngine.getRenderEngine().getScheduledEffectEngine().add((ScheduledTask) new RotateCamera<>(gameEngine, angle));
                        gameEngine.getRenderEngine().getScheduledEffectEngine().add((ScheduledTask) new FadeInTask<>(gameEngine));
                        gameEngine.getRenderEngine().getScheduledEffectEngine().add((ScheduledTask) new PauseTask<>(gameEngine, secondsDelta));

                        gameEngine.getRenderEngine().getScheduledEffectEngine().add((ScheduledTask) new FadeInTask<>(gameEngine, 0.5f));
...
//                        } else
                    ...
                    gameEngine.getRenderEngine().getScheduledEffectEngine().add((ScheduledTask) new FadeOutTask<>(gameEngine));
                }
            }
        }
