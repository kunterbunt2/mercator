...
public class MercatorCameraTest extends TranslationUtil<LiniarTranslation> {
	...
	private final float factorIncrement = 1;
	...
	private void createCamera() {
		...
		//yaw test
		moveCamera(100f, 0f, 0f);
		printCamera(camera);
		moveCamera(-100f, 0f, 0f);
		printCamera(camera);
		moveCamera(0f, 0f, 100f);
		printCamera(camera);
		moveCamera(0f, 0f, -100f);
		printCamera(camera);
		System.out.println();
	...
	private void printCamera(final MovingCamera camera) {
		System.out.println(String.format("x=%+4.0f y=%+4.0f z=%+4.0f pitch=%+4.0f yaw=%+4.0f", camera.position.x, camera.position.y, camera.position.z, camera.getPitch(), camera.getYaw()));
	}

	@Override
	protected void updateTranslation() {
		//		final MovingCamera camera = sceneClusterManager.getCamera();
