...
public class CustomizedSpriteBatch extends PolygonSpriteBatch {
	...
	public void circle(final TextureRegion region, final float x, final float y, final float radius, final float width, final int edges) {
		...
		/*
		 * float startRadius = radius - width; float endRadius = radius + width; float
		 * startAngle = 0f; float endAngle = (float)Math.PI * 2; float border =
		 * 0.02f;//---between this and the next pie piece float overlapping =
		 * 0.03f;//---used to ensure no artifact are visible where the squares overlap
		 * edges = (int)Math.min( edges, ( endAngle - border - startAngle ) /
		 * overlapping ); if ( !drawing ) throw new IllegalStateException(
		 * "SpriteBatch.begin must be called before draw." ); float[] vertices =
		 * this.vertices; Texture texture = region.texture; if ( texture != lastTexture
		 * ) { switchTexture( texture ); } else if ( idx + edges * 4 * 5 >=
		 * vertices.length ) // flush(); final float u = region.u; final float v =
		 * region.v2; final float u2 = region.u2; final float v2 = region.v; float color
		 * = this.color; int idx = this.idx; for ( int edge = 0; edge < edges; edge++ )
		 * { float angle = startAngle + edge * ( endAngle - startAngle ) / edges; float
		 * nextAngle = startAngle + ( edge + 1 ) * ( endAngle - startAngle ) / edges;
		 * //---Center vertices[idx++] = x + (float)Math.sin( angle ) * startRadius;
		 * vertices[idx++] = y + (float)Math.cos( angle ) * startRadius; vertices[idx++]
		 * = color; vertices[idx++] = u; vertices[idx++] = v; vertices[idx++] = x +
		 * (float)Math.sin( angle ) * endRadius; vertices[idx++] = y + (float)Math.cos(
		 * angle ) * endRadius; vertices[idx++] = color; vertices[idx++] = u;
		 * vertices[idx++] = v2; if ( edge == edges - 1 ) { overlapping = 0.0f; }
		 * vertices[idx++] = x + (float)Math.sin( nextAngle + overlapping ) * endRadius;
		 * vertices[idx++] = y + (float)Math.cos( nextAngle + overlapping ) * endRadius;
		 * vertices[idx++] = color; vertices[idx++] = u2; vertices[idx++] = v2;
		 * vertices[idx++] = x + (float)Math.sin( nextAngle + overlapping ) *
		 * startRadius; vertices[idx++] = y + (float)Math.cos( nextAngle + overlapping )
		 * * startRadius; vertices[idx++] = color; vertices[idx++] = u; vertices[idx++]
		 * = v; } this.idx = idx;
		 */
	}
	...
	public void fillCircle(final TextureRegion region, final float x, final float y, final float radius, final int edges) {
		 ...
		 * region.v2; final float u2 = region.u2; final float v2 = region.v; float color
		 * = this.color; int idx = this.idx; for ( int edge = 0; edge < edges; edge++ )
		 * { float angle = (float)( edge * Math.PI * 2 / edges ); float nextAngle =
		 * (float)( ( edge + 1 ) * Math.PI * 2 / edges ); //---Center vertices[idx++] =
		 * x; vertices[idx++] = y; vertices[idx++] = color; vertices[idx++] = u;
		 * vertices[idx++] = v; vertices[idx++] = x + (float)Math.sin( angle ) * radius;
		 * vertices[idx++] = y + (float)Math.cos( angle ) * radius; vertices[idx++] =
		 * color; vertices[idx++] = u; vertices[idx++] = v2; vertices[idx++] = x +
		 * (float)Math.sin( nextAngle + 0.05 ) * radius; vertices[idx++] = y +
		 * (float)Math.cos( nextAngle + 0.05 ) * radius; vertices[idx++] = color;
		 * vertices[idx++] = u2; vertices[idx++] = v2; vertices[idx++] = x;
		 * vertices[idx++] = y; vertices[idx++] = color; vertices[idx++] = u;
		 * vertices[idx++] = v; } this.idx = idx;
		 */
	}
	...
	public void fillPie(final TextureRegion region, final float x, final float y, final float startRadius, final float endRadius, final float startAngle, final float endAngle, final int edges) {
		...
		/*
		 * float border = 0.02f;//---between this and the next pie piece float
		 * overlapping = 0.03f;//---used to ensure no artifact are visible where the
		 * squares overlap edges = (int)Math.min( edges, ( endAngle - border -
		 * startAngle ) / overlapping ); if ( !drawing ) throw new
		 * IllegalStateException( "SpriteBatch.begin must be called before draw." );
		 * float[] vertices = this.vertices; Texture texture = region.texture; if (
		 * texture != lastTexture ) { switchTexture( texture ); } else if ( idx + edges
		 * * 4 * 5 >= vertices.length ) // flush(); final float u = region.u; final
		 * float v = region.v2; final float u2 = region.u2; final float v2 = region.v;
		 * float color = this.color; int idx = this.idx; endAngle -= border; for ( int
		 * edge = 0; edge < edges; edge++ ) { float angle = startAngle + edge * (
		 * endAngle - startAngle ) / edges; float nextAngle = startAngle + ( edge + 1 )
		 * * ( endAngle - startAngle ) / edges; //---Center vertices[idx++] = x +
		 * (float)Math.sin( angle ) * startRadius; vertices[idx++] = y +
		 * (float)Math.cos( angle ) * startRadius; vertices[idx++] = color;
		 * vertices[idx++] = u; vertices[idx++] = v; vertices[idx++] = x +
		 * (float)Math.sin( angle ) * endRadius; vertices[idx++] = y + (float)Math.cos(
		 * angle ) * endRadius; vertices[idx++] = color; vertices[idx++] = u;
		 * vertices[idx++] = v2; if ( edge == edges - 1 ) { overlapping = 0.0f; }
		 * vertices[idx++] = x + (float)Math.sin( nextAngle + overlapping ) * endRadius;
